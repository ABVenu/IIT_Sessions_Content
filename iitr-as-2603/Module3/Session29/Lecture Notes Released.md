...
Loading PDFs, **chunk-size tuning**, richer metadata on real policies, prompt assembly, and a full end-to-end RAG script are covered in later work on the same track.

---

## RAG Retriever — Where This Session Fits

Before diving into vectors, we recapped the **retriever half** of RAG from the previous session:

1. **Load and chunk** documents — split large files into searchable pieces
2. **Embed chunks** — each chunk becomes a vector of numbers (chunk embedding)
3. **Store in vector DB** — embeddings saved with text and metadata
4. **User asks a question** — the query becomes a **query embedding**
5. **Similarity search** — find **top-k** nearest chunk vectors
6. **Retrieve context** — those chunks feed the LLM in a later generation step

**Today's focus** is steps 2–5: embeddings, Chroma storage, and semantic search. **Chunking real PDFs** and **grounded generation** come in later work on the same track.
...
- **Real-Life Example:** Google's **Word2Vec** showed that *king − man + woman ≈ queen* in vector space — meaning becomes math. In our lab, two refund-related sentences land near each other for the same reason.
...
### Where Embeddings Started — Word2Vec

- **Historical note:** In **2013**, Google released **Word2Vec** — one of the first widely used embedding models.
- Words became **word vectors** — fixed lists of numbers where similar words sit near each other in space.
- **Famous demo:** *king − man + woman ≈ queen* — you can do **math with meaning** when words are vectors.
- Tools like **TensorFlow Projector** plot thousands of word vectors; nearby points are semantically related words.
- From **word embeddings**, models evolved to **sentence** and **chunk embeddings** — the same idea at longer-text level.
...
### Plotting Chunks on a Coordinate Map

- Imagine three chunks plotted in 2D: **C1** and **C2** (both about HR leave policy) sit **close**; **C3** (compensation policy) sits **far away**.
- You cannot draw 384 dimensions on paper — but the idea is identical: **closer points = closer meaning**.
- **Important:** Individual numbers in a vector (e.g. `0.7`, `0.8`) are **not human-readable** — they are hidden neural features. Collectively they encode meaning; alone they tell you nothing.

### Distance Formulas

- Once vectors are plotted, **distance** measures how far apart two points are.
- Common options: **Euclidean distance** (straight-line gap) and **cosine distance** (angle between vectors).
- **Cosine distance** is usually preferred for text embeddings.
- In **Chroma**, a **lower distance** between query and stored chunk usually means **closer meaning**.
...
- **Real-world link:** Modern **Google search** (AI Overviews) and **Perplexity** both use retrieval and semantic similarity over indexed content — the same retrieve-then-answer idea as RAG, at web scale. Historically search was mostly **keyword-based** (SEO, exact word match); today a large part is **meaning-based**.
...
- **Other tools:** Production systems also use **Pinecone**, **PGvector** (Postgres), and cloud vector stores. This course lab uses **Chroma** — understand Chroma and you can move to others easily.
...
- **Metadata filtering:** Tags like `category: "returns"` let you narrow search before similarity — e.g. only compare against return FAQs when the question is about refunds. This improves **efficiency** on large collections.
...
- **Vector DBs never say "I don't know"** — they always return the **mathematically closest** vectors, even when nothing in the store truly answers the question. That is why **grounding** (answer only from retrieved context, or admit "not found") matters in the full RAG pipeline.

Tuning **k** and advanced metadata filters on real PDFs are covered in later RAG work on the same track.
...
|---|---|
| **Chunking and document preparation** | Longer policies than one FAQ line — **chunk size**, **overlap**, **`source_id` / `page`**, persist with the **same Chroma path** |
...
- **Word2Vec** showed that words — and later sentences — are **coordinates in meaning-space**; closer vectors mean closer meaning.
- **Embeddings** are semantic coordinates; **rank** and **distance** judge matches — not exact word overlap. **Cosine distance** is usually preferred; in Chroma, **lower distance** often means nearer meaning.
...
- Vector DBs return the **closest match by math**, not business correctness — inspect **top-k** text yourself; weak or missing chunks cause weak Rank 1.

Later work **chunks real documents**, attaches richer metadata, and persists them in a new collection using the same embed-and-upsert pattern.
...
| **Word2Vec** | Concept | Google's 2013 model — words as vectors; origin of modern embeddings |
| **Cosine distance** | Concept | Preferred distance metric for text similarity |
| **Grounding** | Concept | Answer only from retrieved context — vector DB alone cannot refuse |
