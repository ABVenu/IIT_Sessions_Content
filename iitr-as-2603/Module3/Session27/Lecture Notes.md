...
Today you explore **open-source LLMs** on your own machine: install **Ollama**, run a **light model**, call it from **Python**, compare **local vs Ollama Cloud** on the same prompt, and finish with **one script** that switches modes via **`.env`** — secrets stay out of Git.
