...
└── requirements.txt      # Pinned package list for teammates
```

### `.env` — what belongs inside

For **local Ollama**, you usually need **no API key** on `localhost`. You still centralise **model name**, **host**, and **temperature** so one change updates every script.

For **Ollama Cloud**, create an API key from **ollama.com** → **Settings** → **Keys**, then store it in **`OLLAMA_API_KEY`** inside **`.env`**.
...
Set **`USE_CLOUD=1`** when you want cloud mode and have a valid **`OLLAMA_API_KEY`**.
...
List every package your script needs so teammates install the same stack with one command:
...
- **Common mistake:** Forgetting to update **`requirements.txt`** after adding a new import — your script works locally but fails on a teammate's laptop.
...
Practical ranges used in class:

| Range | Label | Behaviour |
| --- | --- | --- |
| **0.0 – 0.3** | Low | Uses common, predictable words — good for teaching demos |
| **0.4 – 0.7** | Medium | Balanced variety |
| **0.8 – 1.0** | High | More creative and random — may use rare words or drift off-topic |

For classroom demos, **`0.3`** keeps answers focused. Try **`0.9`** once to see how word choice and creativity change while the **chain structure** stays the same.
...
4. **`ollama run <tag>`** opens an interactive chat in the terminal — useful to confirm the model works before running Python.
5. **Common mistake:** `connection refused` on port **11434** — Ollama is not running.
6. **Common mistake:** Wrong model downloaded — if your script expects `qwen2.5:0.5b` but you only pulled another tag, pull the exact tag first.

### Switching model providers

LangChain's main benefit is **swappable** model bindings. The chain spine stays the same; you change the import and class name:

| Provider | Typical class | Package |
| --- | --- | --- |
| **Ollama (local)** | `ChatOllama` | `langchain-ollama` |
| **Groq** | `ChatGroq` | `langchain-groq` |
| **OpenAI** | `ChatOpenAI` | `langchain-openai` |

Rather than rewriting HTTP glue, you update the model Runnable and its provider package — the **prompt** and **parser** steps stay connected with the same **`|`** pipe.
...
### RTF pattern in system prompts

A strong system message often follows **RTF** — **Role**, **Task/constraint**, and **Format/instruction**:

- **Role:** *"You are a beginner-friendly programming instructor."*
- **Task / constraint:** *"Explain concepts in simple language with short bullet points."*
- **Format / instruction:** *"Avoid long introductions."*

This keeps the model focused without rewriting the whole prompt for every topic.
...
Another useful picture: a **college canteen queue**. You pick items at one counter, then the next, then the next — prompt first, then the LLM, then the output parser. Each step passes its result forward in a fixed order.
...
- **Optional gating:** The validation checks in **`hello_chain.py`** are a teaching aid. If a response fails only the word-limit check but still reads well, the **chain itself** is working — read the output to confirm.
...
- A **venv** plus a clear **folder layout** (**.env**, **`.gitignore`**, **`requirements.txt`**, **`hello_chain.py`**) keeps LangChain work reproducible for the whole cohort.
- **ChatOllama** binds LangChain to Ollama using **model**, **base_url**, and **temperature** from your **Ollama setup block** in `.env` — not scattered hard-coded strings.
...
- **Temperature** (low **0.3** vs high **0.9**) changes creativity and word choice; the **chain structure** stays the same.
...
| `pip install -r requirements.txt` | Install all packages listed for the project |
...
| **`USE_CLOUD`** | `0` = local Ollama; `1` = cloud mode with API key |
| **ChatOllama** | LangChain Runnable for Ollama chat models |
| **ChatGroq / ChatOpenAI** | Swappable provider Runnables — same LCEL pipe pattern |
| **ChatPromptTemplate** | Reusable system/human message template |
| **RTF** | Role, task/constraint, format — system prompt pattern |
...
| `ollama pull <tag>` | Download the exact model your script expects |
| `ollama run <tag>` | Interactive terminal chat with the model |
