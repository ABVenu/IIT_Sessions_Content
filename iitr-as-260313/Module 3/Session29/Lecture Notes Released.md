...
Every **AI agent** has three core parts: an **LLM** (the brain), **memory** (short-term or long-term context), and **tools** (third-party functions the LLM can call to get real work done). Today’s agent does not use memory yet — each query runs independently — but the tool-calling pattern is the same one used in support bots, order assistants, and workflow automation.
...
Agent functions live in a separate package from core LangChain utilities. Install it before running the lab:
...
In production, each tool function would call a real API (for example, a courier partner for delivery status or a payment gateway for refund data) instead of reading from an in-memory dictionary. The agent pattern stays the same — only the tool body changes.
...
Most executor parameters have **default values** and are not strictly required:

- Only **`create_tool_calling_agent`** and **`AgentExecutor`** creation is mandatory.
- `handle_parsing_errors`, `max_iterations`, `verbose`, and `return_intermediate_steps` can be omitted — defaults will apply.
- Setting `handle_parsing_errors=False` can cause the run to crash if a parsing error occurs mid-loop.
- Setting `return_intermediate_steps=False` hides step traces; if your code prints those steps, the program may fail when steps are missing.
...
## Model Choice and Test Reliability

The same agent code can behave differently depending on which LLM you use.

- A smaller local model (for example, Quen) may call the wrong tool or fail edge-case tests more often.
- A stronger model (for example, Llama 3.1 or a cloud model) usually passes more test cases.
- **Pass/fail on the cohort test pack is model-dependent** — a failing test may reflect model reasoning, not a bug in your tool code.
- Keep **`temperature=0`** during validation so results stay consistent across runs.

When a test fails, inspect intermediate steps first. That tells you whether the problem is tool selection, wrong arguments, or the final answer wording.
...
- Results may differ by model — re-run with a stronger model if edge cases fail unexpectedly.
