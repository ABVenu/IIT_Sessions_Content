...
### Extra in-Class Concept — Ingestion and Index Refresh Lifecycle

In class, you also discussed how retrieval quality depends on a clean and regularly updated index. This lifecycle runs mostly in the background, and it decides whether your "grounded" answers stay current over time.

| Phase | What the system does | Why it matters for students |
|---|---|---|
| **Scrape / collect** | Pull policy text from PDFs, URLs, CMS pages, or docs | Missing source data means missing evidence later |
| **Clean and normalize** | Remove noise, fix formatting, standardize text | Cleaner text gives better chunks and embeddings |
| **Chunk and embed** | Split documents into searchable parts and convert to vectors | Retrieval can match meaning for real user phrasing |
| **Store and index** | Save chunks + vectors in the vector database | Makes top-k search fast during user queries |
| **Scheduled refresh** | Re-run ingestion daily/hourly based on policy change frequency | Prevents stale policy answers after updates |

- **Official Definition:** **Ingestion pipeline** is the pre-retrieval process that prepares source documents for search by collecting, cleaning, chunking, embedding, and indexing them.
- **In Simple Words:** This is the **kitchen prep** before serving — if prep is poor, even a good chef cannot fix the dish fully.
- **Real-Life Example:** A grocery store shelf must be restocked daily; otherwise staff still reads old price tags and gives wrong billing answers.

**Integrated learning point:** Even a well-prompted LLM cannot compensate for stale or badly chunked indexes. Good RAG needs both reliable **online retrieval** and disciplined **offline ingestion maintenance**.
...
| **Ingestion pipeline** | Offline workflow that prepares and refreshes searchable document index |
| **Scheduler / refresh job** | Timed trigger that reruns ingestion to keep policy index current |
