# Final Capstone — Nykaa Domain Support Agent (CrewAI)

**Track: E-commerce & Retail (Nykaa).** **Estimated duration: 14 days.**
...
Nykaa's customer-support team wants an agent that can answer policy questions and
check a specific order's status, so agents (and eventually customers) get instant,
grounded answers instead of escalating everything to a human. You are building that
agent — and building it to a standard where it could pass a real production
governance review, not just a happy-path demo.
...
- State at the top of your `README.md` that you completed the **Nykaa (E-commerce &
  Retail)** track, and include the exact dataset-design choices you made in Part 1
...
**Required knowledge-base topics (≥12 documents, ≥2-5 sentences each):** return window
by product category, COD refund timelines, delivery SLAs, reverse-pickup eligibility,
warranty terms by category, order-cancellation policy, loyalty-points redemption
policy, payment-failure/retry policy, size-exchange policy, damaged-item claim process,
international shipping restrictions, and the customer-support escalation matrix.

**Guardrail-relevant PII fields:** phone number and payment-card last-4-digits are
fixed-format and must be demonstrated masked by your input-side guardrail (Task 10).
Customer name and delivery address are free text with no reliable pattern to match
under a keyless, `MOCK_LLM`-only masker — acknowledged as out of scope for masking;
use only fabricated examples for both, never anyone's real data.

## Part 1 — Dataset Design & RAG Core (30 marks)

**Tasks**

1. **Design and validate your own order dataset.** Write a seeded, deterministic
   Python generator (`dataset.py`) producing an `ORDERS` list of ≥40 order records.
   Use every category and status value given above at least once (you may add more
   values). Each record needs: `record_id`, `category`, `status`, `order_value_inr`
   (choose a realistic price range and state your reasoning in one sentence),
   `days_since_created` (integer, 0–30), and `delayed_shipment` (boolean). Print and
   report: the count per category (every given category ≥3 records), the count per
   status (every given status ≥1 record), and the percentage of records with
   `delayed_shipment=True` (must land between 10% and 30% — if your first random draw
...
6. **Build the second tool with a designed escalation score.** Using `dataset.py`
   from Task 1, implement `check_order_status(record_id: str) -> dict` returning the
   order's `status`, `order_value_inr`, and a **designed** `escalation_score` in
   `[0, 1]` that combines `delayed_shipment` with a normalized recency signal derived
   from `days_since_created`. State your exact formula and the threshold above which
   you recommend escalation, justified using your own dataset's distribution (e.g.
   "this threshold corresponds to the 80th percentile of `days_since_created` in my
   generated data").
7. **Build a CrewAI crew.** Construct a crew with ≥3 agents — at minimum a Retrieval
   ...
   `check_order_status` tool (Task 6), and a Response Composer agent that combines
   their outputs into one final draft answer — and run it via `.kickoff()`.
...
15. **Apply the four-layer AI-governance model.** At the **Application layer**,
    ...
    `check_order_status` — no other agent in your crew may call it directly, and you
    must demonstrate this restriction (e.g. show that wiring the tool to another
    agent is blocked or simply never wired, with a one-paragraph explanation of the
    guard). Classify your own system's risk level using the Low/Medium/High risk
    scheme (Low: summarization/transcription; Medium: code generation/customer
    support tickets; High: medical data, hiring decisions, financial data), with a
    one-paragraph justification. At the **Runtime layer**, enforce a
    per-request token/cost budget cap and demonstrate a deliberately oversized
    request being rejected rather than silently exceeding the budget.
...
**Part 1**
- `dataset.py` generates ≥40 orders meeting every stated structural threshold
  (category counts, status coverage, `delayed_shipment` percentage band), with your
  design choices stated in `README.md`.
...
**Part 2**
- `check_order_status` correctly looks up a record and correctly computes a designed,
  justified `escalation_score` (not a bare boolean OR).
- The CrewAI crew has ≥3 agents (Retrieval, Lookup, Composer at minimum); both the RAG
  tool and the `check_order_status` tool are demonstrably invoked on different
  queries via `.kickoff()`.
