# Final Capstone — Naukri.com Domain Support Agent (CrewAI)
...
and building it to a standard where it could pass a real production governance
review, not just a happy-path demo.
...
7. **Build a CrewAI crew.** Construct a crew with ≥3 agents — at minimum a Retrieval
   ...
   `check_job_application_status` tool (Task 6), and a Response Composer agent that
...
15. **Apply the four-layer AI-governance model.** At the **Application layer**,
    ...
    `check_job_application_status` — no other agent in your crew may call it
    ...
    At the **Runtime layer**, enforce a per-request token/cost budget cap
    and demonstrate a deliberately oversized request being rejected rather than
...
- The CrewAI crew has ≥3 agents (Retrieval, Lookup, Composer at minimum); both the RAG
  tool and the `check_job_application_status` tool are demonstrably invoked on
