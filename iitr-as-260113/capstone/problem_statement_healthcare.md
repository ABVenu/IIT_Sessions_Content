# Final Capstone — Practo Domain Support Agent (CrewAI)

**Track: Healthcare (Practo).** **Estimated duration: 14 days.**
...
Practo's patient-experience team wants an agent that answers clinic-policy questions
and checks a specific appointment's status, so patients get instant, accurate answers
about scheduling, fees, and follow-ups. You are building that agent — and building it
to a standard where it could pass a real production governance review, not just a
happy-path demo.
...
- State at the top of your `README.md` that you completed the **Practo (Healthcare)**
  track, and include the exact dataset-design choices you made in Part 1 Task 1
  (seed, category/status weights, amount range) so the grader can reproduce your
...
appointment-booking policy, cancellation/rescheduling window, consultation-fee
structure by specialty, insurance-claim process, prescription-refill policy, lab-test
turnaround times, telemedicine eligibility, emergency-visit protocol, patient-data
privacy policy, follow-up-visit discount policy, second-opinion process, and
home-visit eligibility.

**Guardrail-relevant PII fields:** contact number is fixed-format and must be
demonstrated masked by your input-side guardrail (Task 10). Patient name,
diagnosis/condition, and insurance ID are free text or have no universal format to
match under a keyless, `MOCK_LLM`-only masker — acknowledged as out of scope for
masking; use only fabricated examples for all three, never real medical data.

## Part 1 — Dataset Design & RAG Core (30 marks)

**Tasks**

1. **Design and validate your own appointment dataset.** Write a seeded,
   deterministic Python generator (`dataset.py`) producing an `APPOINTMENTS` list of
   ≥40 records. Use every category and status value given above at least once (you
   may add more values). Each record needs: `record_id`, `category`, `status`,
   `consultation_fee_inr` (choose a realistic range and state your reasoning in one
   sentence), `days_since_created` (integer, 0–30), and `follow_up_required`
   ...
   of records with `follow_up_required=True` (must land between 10% and 30% — if
   your first random draw doesn't land there, change your seed or weights and
...
6. **Build the second tool with a designed escalation score.** Using `dataset.py`
   from Task 1, implement `check_appointment_status(record_id: str) -> dict`
   returning the appointment's `status`, `consultation_fee_inr`, and a **designed**
   `escalation_score` in `[0, 1]` that combines `follow_up_required` with a
...
7. **Build a CrewAI crew.** Construct a crew with ≥3 agents — at minimum a Retrieval
   ...
   `check_appointment_status` tool (Task 6), and a Response Composer agent that
...
15. **Apply the four-layer AI-governance model.** At the **Application layer**,
    ...
    `check_appointment_status` — no other agent in your crew may call it directly,
    and you must demonstrate this restriction (e.g. show that wiring the tool to
    another agent is blocked or simply never wired, with a one-paragraph explanation
    of the guard). Classify your own system's risk level using the Low/Medium/High
    risk scheme (Low: summarization/transcription; Medium: code generation/customer
    support tickets; High: medical data, hiring decisions, financial data), with a
    one-paragraph justification. At the
    **Runtime layer**, enforce a per-request token/cost budget cap and demonstrate a
...
**Part 1**
- `dataset.py` generates ≥40 appointments meeting every stated structural threshold
  (category counts, status coverage, `follow_up_required` percentage band), with
...
**Part 2**
- `check_appointment_status` correctly looks up a record and correctly computes a
...
- The CrewAI crew has ≥3 agents (Retrieval, Lookup, Composer at minimum); both the RAG
  tool and the `check_appointment_status` tool are demonstrably invoked on different
  queries via `.kickoff()`.
