...
Your **ShopEasy support agent** finally has a proper **evaluation harness**. The **runner** executes twenty cases. **results.csv** lands in the team channel: **twelve passes**, **eight failures**. Someone opens a **failure trace** for the GST case — the agent called the **policy retriever** instead of the **calculator**. Clear diagnosis. Everyone feels productive.
...
What if you patch the **calculator tool description** and the GST case passes — but the **CEO email** refusal case starts **fabricating** because the new prompt says *"always try to help"*?
...
|---|---|
| **Wrong tool** | **Tool description patch** — rewrite the tool's name or description so the agent understands *when* to pick calculator vs retriever |
| **Weak retrieval** | **Retrieval tune** — adjust chunk size, number of passages retrieved, or embedding index settings so the right policy text surfaces |
...
This loop is what makes the **upcoming** real-world use-case session credible: when you demo an **HR onboarding agent**, you can say not only *"here are test cases"* but *"here is what we fixed, here is the before/after scoreboard, here is what we chose not to fix yet."*
...
- **Retrieval tune:** Adjusting **chunk size**, **top-k** (how many passages to fetch), or related index settings so retrieval returns **more useful context**.
...
- **Prepare** for **upcoming** work on **real-world use cases** — where the same loop validates an **HR onboarding agent** before a live demo
...
Today's session is the **engineering maturity layer** on top of evaluation. **Upcoming** work applies the full stack — integrated agent, harness, iteration loop — to **cross-industry patterns** and a stakeholder-ready **HR onboarding demo**. Debugging discipline is what makes that demo defensible, not just impressive.

---

## Questions to carry into the session

1. **results.csv** shows three failures labelled **wrong tool** — all arithmetic questions routed to the **retriever**. Two teammates propose fixes: **A)** rewrite the entire system prompt to mention math more often; **B)** only patch the **calculator tool description** to say *"Use for GST, percentages, and numeric calculations — not policy questions."* Which is the more **controlled configuration change**, and how would you prove it worked without breaking **in-domain policy** cases that already pass?
...
Keep these questions in mind. The session turns your **evaluation harness** into an **improvement engine** — measured fixes, visible progress, honest limits — so the agent you demo next is something you **iterated on purpose**, not something you **got lucky with on Friday afternoon**.
