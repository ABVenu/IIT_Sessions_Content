...
## Q4 (MCQ, Easy)
In a tool-calling setup, what is the role of a function decorated with `@tool`?

A. It becomes available for the agent to call as an external action  
B. It automatically retries failed API requests 10 times  
C. It replaces the LLM for all responses  
D. It stores chat history in a database

**Correct Answer:** A

**Answer Explanation:**  
The decorator exposes a Python function as a callable tool that the agent can choose during reasoning.

**Why other options are wrong:**  
- B: Retry behavior is not automatically guaranteed by `@tool`.  
- C: LLM remains central for reasoning and response generation.  
- D: History storage is a separate concern.
...
**Why other options are wrong:**  
- A: Avoiding tools can lead to incorrect/fabricated answers.  
...
High temperature increases randomness; it is not a core safety/debug control.
...
D. Include a case with missing/invalid order ID
...
A. It helps preserve current-run reasoning/tool traces across steps  
