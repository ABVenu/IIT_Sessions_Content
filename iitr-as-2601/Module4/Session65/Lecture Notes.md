...
Capstone is a different product. You will not paste parcel FAQs into a new folder and call it done. You will freeze **Nimbus PayDesk**, draw one page of architecture, implement the **LangGraph** core, and sit a **golden** exam. This meeting is the **build**. The **next** meeting is the only remaining live slot: window, demo, and handover in one go.
...
| **Tools** | Read or write truth | `check_gstin`, `lookup_po`, `retrieve_policy`; diary is `write_trace` |
...
**Deploy path (named today, built next meeting):** local **Streamlit** — the stakeholder window you already practised. Do not open a second HTTP hatch in the core path. Do not start the window today even if the graph is green early.
...
class InvoicePacket(TypedDict, total=False):  # extra keys allowed
    ...
    status: str  # extracted / ok_for_route / ready_to_pay / needs_human
...
Write each run as one **JSON line** (`ticket_id`, `prompt_version`, `status`, `gate`, `trace`) into `logs/paydesk.jsonl`. That is observability you already practised. The eval runner must **call** `write_trace` — `invoke` alone does not create the file.
...
nimbus_paydesk/
  ...
  app/log.py
  data/policy.md
  scripts/seed.py          # sqlite vendors/POs + seed_policy()
...
Use `INSERT OR REPLACE` for seeds so a re-run does not crash. Bound every query with `?`. The live seed is **vendors + POs**. `tickets` / `events` can wait — JSONL is today’s diary.

**Copyable seed (Lab A — run this before the first invoke):**
...
print("seeded sqlite and chroma")  # you should see this before G01
```

**Duplicate habit (not a live gate today):** a twin ticket would stop on `duplicate`. Do **not** add that gate to G01–G03 — those bills have no `bill_date`. Put it on the retro.

```python
# app/log.py — one JSON line per finished graph
import json  # structured diary
from datetime import datetime, timezone  # stamp time
from pathlib import Path  # logs folder


def write_trace(packet: dict) -> None:  # append-only
    Path("logs").mkdir(exist_ok=True)  # otherwise the first write crashes
...
- The **upcoming** meeting opens this file (or a Streamlit expander filled from `trace`) as evidence — and packs the replay kit the same day
...
When you later call Groq, log token counts on the same JSONL row. Do not wait for the next meeting to discover that CLEAN spent a fortune — but also do not block **today’s** golden run on a missing key. Lab labels are allowed for G01–G03.
...
**Live pause — Lab A:** stop adding nodes. Run `python scripts/seed.py`, then one `graph.invoke` on any labelled bill. If the shelf is empty, you will see `empty_policy`. That is correct. Seed, then continue. Do **not** open Streamlit.
...
from app.log import write_trace  # diary the next meeting will open
...
def run_case(case: dict) -> str:  # one exam
    ...
    write_trace(out)  # otherwise “three JSONL lines” never appear
...
**Live pause — Lab B:** sit G01, then G02, then G03. Fix one class (usually empty Chroma). Re-run. Confirm three JSONL lines. Guardrails stay in Python — do not “fix” HIGH by editing the amount.
...
The **upcoming** meeting will put a window on this desk **and** pack the README. A pretty screen on a graph that still pays HIGH is worse than an ugly terminal that stops. Do not skip the graph to start the handover docs.
...
## Build Lab Order (This Meeting)

This meeting is long enough to **leave with a passing graph**, not only a drawing. Use the extra time as lab.

**Need to do before you pack the laptop:**

1. Freeze the sentence and the dummy GSTINs
2. Create the folder map and `data/policy.md`
3. Seed sqlite3 (Kaveri, Nilgiri — never `99INVALID`) and `seed_policy()`
4. Wire extract → policy → route
5. Sit G01, then G02, then G03
6. Fix one blocking class (usually empty Chroma) and re-run

**If you finish early:** sit G04 INJECT (HIGH text says ignore rules — still `amount_gate`). Do **not** start Streamlit. Do **not** start the README. The next meeting is one slot for the counter **and** the replay kit. Decorating first is how HIGH sneaks through.

**Carry to the next meeting:** running `graph.invoke`, JSONL lines, seeded Chroma, `eval/run_golden.py`.
...
The **upcoming** meeting polishes the **counter** (Streamlit, a demo-path **cost** note, a short live story with **traces**) **and** packs the file (README, checklist, partner review). One meeting. Do not decorate a graph that still fails HIGH.
...
| Streamlit | Next meeting | Stakeholder window and handover together |
