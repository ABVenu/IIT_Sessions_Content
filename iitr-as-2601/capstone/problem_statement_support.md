# Final Capstone — Ola Domain Support Agent (LangGraph)
...
standard where it could survive a real production incident, not just a happy-path
demo.
...
6. **Build the second tool with a designed escalation score.** Using `dataset.py`
   ...
   `escalation_score` in `[0, 1]` that combines `escalated` with a normalized recency
   signal derived from `days_since_created`. State your exact formula and the
   threshold above which you recommend escalation, justified using your own
...
7. **Build the LangGraph agent.** Construct a LangGraph graph with ≥4 nodes and ≥1
   ...
   (Task 3–5) or your `check_support_ticket_status` tool (Task 6) based on query
...
14. **Expose your lookup tool via MCP.** Install `fastmcp` (`pip install fastmcp` —
    ...
    `check_support_ticket_status` (with a proper docstring) as an MCP tool,
...
**Part 4**
- `check_support_ticket_status` is successfully callable via a real MCP
