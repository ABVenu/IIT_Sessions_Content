# Final Capstone — Nykaa Domain Support Agent (LangGraph)
...
agent — and building it to a standard where it could survive a real production
incident, not just a happy-path demo.
...
7. **Build the LangGraph agent.** Construct a LangGraph graph with ≥4 nodes and ≥1
   ...
   (Task 3–5) or your `check_order_status` tool (Task 6) based on query intent —
   demonstrate both routes firing on different sample queries.
...
14. **Expose your lookup tool via MCP.** Install `fastmcp` (`pip install fastmcp` —
    ...
    `check_order_status` (with a proper docstring) as an MCP tool, launching a
    private MCP server locally. `fastmcp`'s HTTP transport mounts at `/mcp` by
    default (e.g. `http://127.0.0.1:8000/mcp`), not the bare host:port root — point
    your client at that path. Write a **separate** MCP client script (a different
    file/process from your LangGraph agent) that connects to this server and
    successfully calls the tool for at least 2 different record IDs, printing the
...
**Part 4**
- `check_order_status` is successfully callable via a real MCP client-server round
  trip on ≥2 record IDs.
