# Final Capstone — Practo Domain Support Agent (LangGraph)
...
to a standard where it could survive a real production incident, not just a
happy-path demo.
...
7. **Build the LangGraph agent.** Construct a LangGraph graph with ≥4 nodes and ≥1
   ...
   (Task 3–5) or your `check_appointment_status` tool (Task 6) based on query intent
...
14. **Expose your lookup tool via MCP.** Install `fastmcp` (`pip install fastmcp` —
    ...
    `check_appointment_status` (with a proper docstring) as an MCP tool, launching a
...
**Part 4**
- `check_appointment_status` is successfully callable via a real MCP client-server
  round trip on ≥2 record IDs.
