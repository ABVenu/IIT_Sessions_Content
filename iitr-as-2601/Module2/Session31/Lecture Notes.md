...
In the last session, we introduced our first classification algorithm — **Logistic Regression** — and learned how it predicts the probability of a student passing or failing. We also built our first diagnostic tool, the **Confusion Matrix**, which broke down every prediction into four categories: True Positives, True Negatives, False Positives, and False Negatives. But we only looked at one metric to summarise model quality: **accuracy**.

The problem with accuracy is that it can be deeply misleading. If 95 out of 100 students pass, a model that always predicts "Pass" achieves 95% accuracy — but catches exactly zero failing students. A single number like accuracy hides these failures. This session solves that problem by introducing two new and more powerful classification models, and a complete toolkit of evaluation metrics that reveal the full picture of what a model is truly doing.

---

## Decision Trees: How a Model Can Think in Rules

Before we introduce new metrics, let's build two new classification models. The first is the **Decision Tree** — arguably the most intuitive machine learning algorithm ever designed, because it makes decisions the same way a human would: by asking a series of yes/no questions.
...
- **In Simple Words:** The model looks at your data and learns a flowchart. It asks questions like "Is study_hours > 5.2?" — if yes, go left; if no, go right. It keeps asking more specific questions until it reaches a final answer (Pass or Fail).
- **Real-Life Example:** When a bank employee decides whether to approve a loan, they don't use a formula — they ask questions: "Is the applicant's income above ₹50,000?" → Yes → "Is their credit score above 700?" → No → Reject. A Decision Tree learns exactly this kind of structured question-answer logic from past data.

**Key vocabulary inside a Decision Tree:**

- **Root Node** — The very first question asked, at the top of the tree. It is the single question that most powerfully splits the data into two groups. In our student example, this might be: "Is study_hours > 5.2?"
- **Branch** — Each answer to a question (Yes or No) leads to a branch — a path going left or right down the tree.
- **Internal Node** — Any question node inside the tree that is not the root and not the final answer. Each internal node asks one more feature-based question to further refine the prediction.
- **Leaf Node** — The final node at the bottom of a branch. No more questions are asked here — the model delivers its final class prediction: Pass or Fail.
...
The Decision Tree tries every possible feature and every possible split value (e.g., "study_hours > 3.0", "study_hours > 5.5", etc.) and picks the one that creates the **purest** groups — meaning one group has mostly Pass students and the other has mostly Fail students. This purity is measured using a score called **Gini Impurity** — a group with all-Pass students has Gini impurity of 0 (perfectly pure), while a group that is 50/50 Pass and Fail has the highest impurity. The tree keeps splitting until it reaches maximum depth or until the groups are pure enough.

**The overfitting risk with Decision Trees:**

A tree with unlimited depth will ask so many specific questions that it perfectly memorises every training student — including all the noise and random quirks. On new test students, it will perform poorly. The **`max_depth`** parameter is the primary tool for preventing this: it stops the tree from asking more than a set number of questions, forcing it to learn general patterns rather than memorising individuals.
...
# Simulate exam scores using the same formula as Session 30
scores = (
    40
...
- **`DecisionTreeClassifier(max_depth=4, random_state=42)`** — Creates a Decision Tree limited to 4 levels of questions deep. The `random_state` ensures that when there are tie-breaking decisions during training, the same choice is made every run.
...
- **`tree.feature_importances_`** — A NumPy array with one value per feature. Each value represents the fraction of the total "impurity reduction" that feature contributed across all splits in the tree. Values sum to 1.0. A feature with importance 0.72 was responsible for 72% of all the tree's splitting power.
- **`tree.predict(X_test)`** — Runs each test student through the learned tree: starts at the root, answers each question using the student's feature values, and follows the branches until a leaf node is reached.
- **`tree.predict_proba(X_test)`** — At each leaf node, the tree stores the proportion of training students in that leaf who passed vs failed. These proportions become the probability output. A leaf with 8 passes and 2 fails gives P(Pass) = 0.8 for any student who ends up there.
- **The bar visualisation** — The `"█" * int(importance * 30)` creates a simple visual bar proportional to each feature's importance, making the comparison instantly readable.

**What to observe in the output:**

- `study_hours` should have the highest feature importance — it was the strongest predictor in the data formula.
- `sleep_hours` should show moderate importance.
- `distractions` may show lower importance depending on how the tree splits the data.
- A tree with max_depth=4 may not achieve the highest possible accuracy, but it will generalise better to new data than an unlimited-depth tree.

---

## Random Forest: When Many Trees Are Better Than One

A single Decision Tree, even with controlled depth, has a known weakness: it is sensitive to the specific training data it sees. If you slightly change the training set — add a few students, remove a few — the tree's structure can change significantly. This makes individual trees somewhat fragile. The solution is **Random Forest**.
...
- **In Simple Words:** Instead of building one tree and trusting it completely, Random Forest builds hundreds of trees, each learning from a slightly different version of the data. Then, when predicting a new student, it asks all 100 trees and takes a vote — the majority answer wins. A single tree might be wrong, but 100 trees working together are much harder to fool.
- **Real-Life Example:** Imagine you're trying to decide which cricket player is the best. Instead of asking one expert, you ask 100 cricket analysts. Each analyst has slightly different knowledge and perspective. The consensus of 100 experts is far more reliable than any single opinion — even if each individual expert isn't perfect. That's exactly how Random Forest works.

**Why Random Forest is more accurate and stable than a single tree:**

- **Bagging (Bootstrap Aggregating)** — Each tree in the forest is trained on a different random sample of the training data (drawn with replacement). This means some students appear multiple times in one tree's training set and not at all in another's. This diversity prevents all trees from making the same mistake.
- **Random Feature Selection** — At each split in each tree, only a random subset of features is considered. This further forces diversity between trees, so they don't all learn the same patterns.
- **Majority Voting** — For a final prediction, each tree casts its vote. The class with the most votes wins. If 80 trees say Pass and 20 say Fail, the final prediction is Pass.

**The trade-off with Random Forest:**

A single Decision Tree is very easy to visualise and explain — you can literally draw the flowchart of its decisions. Random Forest with 100 trees cannot be visualised in the same way — it is a "black box" model where the reasoning is distributed across hundreds of trees. In return for this loss of direct interpretability, you gain substantially higher accuracy and robustness. In real projects, Random Forest is almost always the better choice unless explainability to non-technical stakeholders is a strict requirement.
...
# Predict class labels for the test students — majority vote across 100 trees
...
- **`RandomForestClassifier(n_estimators=100, max_depth=4, random_state=42)`** — Creates a forest of 100 individual Decision Trees. `n_estimators` controls how many trees are trained. More trees generally means higher accuracy and stability, but also longer training time.
...
- **`rf.predict_proba(X_test)[:, 1]`** — Returns the average P(Pass) across all 100 trees for each test student. Averaging probabilities from 100 trees produces smoother, more reliable confidence scores than a single tree can provide.
...
Now that we have three classifiers — Logistic Regression (from Session 30), Decision Tree, and Random Forest — we need better tools to compare them. Accuracy is the starting point, but it often tells an incomplete story. We need **Precision**, **Recall**, and **F1 Score**.

**Why accuracy alone is not enough:**

Consider a school where 90% of students pass. A model that always predicts "Pass" achieves 90% accuracy — but it never correctly identifies a failing student. It has 0% usefulness for the task of catching students at risk. Yet accuracy says it's doing great. Precision, Recall, and F1 Score are designed to catch exactly this kind of misleading model.
...
- **In Simple Words:** Of all the students the model said "Pass" — what fraction genuinely passed? If the model said "Pass" 80 times and 72 of those students really did pass, Precision = 72/80 = 0.90 (90%).
...
- High precision = When the model says "Yes" (positive), it is usually right. Low false positive rate.
...
- Precision matters most when **false positives are costly** — e.g., falsely approving loans for people who will default, or sending unnecessary aggressive medical treatment to healthy patients.
...
- **In Simple Words:** Of all the students who genuinely passed, what fraction did the model correctly identify? If 100 students actually passed and the model correctly predicted 85 of them, Recall = 85/100 = 0.85 (85%). The other 15 were missed (False Negatives).
- **Real-Life Example:** A security camera system detects 90 out of 100 actual intruders. Recall = 90%. The other 10 intruders slipped through undetected. Recall measures: "Of all the real events that happened, how many did I catch?"

**What Recall tells you:**
- High recall = The model is very good at catching real positives. Low false negative rate.
- Low recall = The model is missing too many real positives. Many genuine cases are going undetected.
...
Precision and Recall are in constant tension with each other. If you make the model more generous (lower threshold → predicts positive more often), Recall goes up (you catch more real positives) but Precision goes down (more false alarms). If you make the model more strict (higher threshold → predicts positive less often), Precision goes up (fewer false alarms) but Recall goes down (you miss more real positives).
...
- **In Simple Words:** F1 takes the middle ground between Precision and Recall. If your model has perfect Precision (1.0) but terrible Recall (0.1), the F1 score will be low — penalising the extreme imbalance. A high F1 score means both Precision and Recall are reasonably high at the same time.
...
# Import confusion_matrix to see the full breakdown
from sklearn.metrics import confusion_matrix

# Import classification_report for a clean formatted summary
from sklearn.metrics import classification_report

# (All model predictions from above are reused: y_pred_tree, y_pred_rf)
# We also need Logistic Regression for comparison — re-train it here
from sklearn.linear_model import LogisticRegression

# Train Logistic Regression on the same training data
...
- **`precision_score(y_true, y_pred, zero_division=0)`** — Computes TP / (TP + FP). The `zero_division=0` argument handles edge cases where no positive predictions are made at all (which would cause a division-by-zero error). It returns 0.0 in that case instead of crashing.
- **`recall_score(y_true, y_pred, zero_division=0)`** — Computes TP / (TP + FN). The same zero_division protection applies.
- **`f1_score(y_true, y_pred, zero_division=0)`** — Computes `2 × Precision × Recall / (Precision + Recall)`. If both are zero (model predicts no positives), it safely returns 0.0.
- **`cm.ravel()`** — Flattens the 2×2 confusion matrix into a single array `[TN, FP, FN, TP]` in that fixed order. This is a clean shortcut to extract all four cells at once without using `cm[0][0]`, `cm[0][1]` etc. individually.
- **`evaluate_classifier()` function** — A reusable helper that wraps the full evaluate workflow. Defining it once and calling it for each model avoids repetition and ensures all models are evaluated identically.
- **`classification_report()`** — Produces a formatted table from scikit-learn that shows Precision, Recall, and F1 for each class separately, plus a weighted average. The `target_names` argument replaces the default "0" and "1" labels with readable names.

**Reading the classification_report output:**

The report shows metrics for both classes — Fail (0) and Pass (1). This is important: a model could be excellent at predicting Pass but poor at predicting Fail. The report breaks it down class by class. The "support" column shows how many actual students belonged to each class in the test set.

---

## ROC-AUC: Measuring How Well the Model Separates the Two Classes

Precision, Recall, and F1 are all computed at a **specific threshold** (usually 0.5). But a model's probability scores carry information that a single threshold cannot fully capture. **ROC-AUC** evaluates the model across all possible thresholds at once — giving you a threshold-independent view of quality.
...
- **In Simple Words:** Imagine slowly lowering the decision threshold from 1.0 (predict almost no one as Pass) all the way down to 0.0 (predict everyone as Pass). At each threshold, you compute: how many real passes did we catch (TPR)? and how many false alarms did we create (FPR)? The ROC curve draws a line through all these points.
...
AUC is computed from probabilities and ranks — it doesn't depend on any specific threshold. This makes it immune to the class imbalance problem. A model that always predicts "Pass" achieves high accuracy on imbalanced data but has an AUC of exactly 0.5 — clearly revealing it has no real predictive power.
...
- **`roc_auc_score(y_test, y_prob)`** — Computes the AUC directly from the actual labels and the model's probability scores. Crucially, it uses the raw probabilities — not the class labels — so it is completely threshold-independent.
- **`roc_curve(y_test, y_prob)`** — Returns three arrays: `fpr` (False Positive Rates), `tpr` (True Positive Rates / Recall), and `thresholds`. Each triplet `(fpr[i], tpr[i], thresholds[i])` represents one point on the ROC curve — what the FPR and TPR would be if you used `thresholds[i]` as your decision threshold.
...
The default threshold is 0.5 — but it is not always the best one. **Threshold Tuning** is the process of systematically finding the probability cut-off that meets a specific business requirement, rather than blindly using the default.
...
- **In Simple Words:** The model gives you probabilities. The threshold is a dial you turn. Ask: "What threshold gives the best F1?" or "What threshold ensures Recall ≥ 90%?" — then use the data to find the answer.
