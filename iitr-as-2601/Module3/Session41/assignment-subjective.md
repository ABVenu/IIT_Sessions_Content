...
2. Use `HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")`.  
3. Persist with `Chroma.from_documents(..., persist_directory="./mini_report_db")`.  
4. Print total documents stored.

### Step 2 — Configure retrieval

1. Reload the store with `Chroma(persist_directory="./mini_report_db", embedding_function=embeddings)`.  
2. Create a retriever with `search_type="similarity"` and `search_kwargs={"k": 3}`.
...
- `build_user_message(user_query, retrieved_chunks)` that returns:
...
Implement `generate_answer(system_message, user_message)` using **Groq**:

- Model: `llama-3.3-70b-versatile` (or another Groq chat model you have access to)  
- `temperature=0`  
- Read `GROQ_API_KEY` from the environment

Implement `rag_answer(user_query, retriever)` that returns a dict with keys: `answer`, `retrieved_chunks`, `user_message`.
...
1. Build `Document` objects from `CORPUS` and persist a small Chroma index with the same embedding model used at query time.  
2. Configure a similarity retriever with `k=3`.  
...
def print_retrieval_trace(user_query, retrieved_chunks):
    ...
    for chunk in retrieved_chunks:
        print(f"Chunk {chunk['index']} | metadata: {chunk['metadata']}")
...
def run_query(label, question, retriever):
    ...
    print_retrieval_trace(question, result["retrieved_chunks"])
    print("\n--- Generated answer ---")
    print(result["answer"])
...
- Rebuild the index once, then load with `Chroma(persist_directory=...)` on later runs instead of calling `from_documents` every time.  
- Add a printed two-column grounding table mapping claims to `source_id` / `page` for Query A.  
- Swap Groq for another OpenAI-style chat API if you keep the same system/user message shape and `temperature=0`.
