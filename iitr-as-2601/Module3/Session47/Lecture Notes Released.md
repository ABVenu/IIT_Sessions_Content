...
> **Real-world note:** A RAG app shines on **large, real manuals** too. A common industry example is a **support assistant** built on an engineering product manual (hundreds of pages), where a technician asks *"the drive tripped with fault code 2310 — what should I check?"* and gets an instant grounded answer instead of calling a support line.
...
- **Fixed-size vs semantic:** the simplest method cuts every N characters in order (**fixed-size**); a more advanced **semantic chunking** keeps related ideas together in one chunk.
...
- **Keep it updated:** just like an orders or employee database, the vector DB must be refreshed when documents are **added or removed** — stale data returns wrong chunks.
...
- **Top-k vs threshold:** instead of a fixed **k**, you can keep every chunk above a **similarity (distance) threshold**; **top-k is more stable** and more widely used, so it stays the default here.
- **Why not too few or too many:** too few chunks = missing context; too many chunks = **higher cost** and **more private data** sent to the LLM.
...
You used **Chroma** in this build, but it helps to know the main alternatives and when each fits.

- **Official Definition:** A **vector database** stores embeddings and runs **nearest-neighbour search** to find the most similar chunks for a query.
- **In Simple Words:** It is a **smart library** that finds notes by **meaning**, not just by matching words.
- **Real-Life Example:** You ask a librarian for *"books about saving money"* and get personal-finance titles — even if the word "money" is not in the title.

| Vector DB | Best for |
|---|---|
| **Chroma** | Open-source, self-hosted; includes storage **and** metadata — great for learning and POCs |
| **Pinecone** | Fully **managed** cloud service; less setup but less control |
| **FAISS** | **Index/search only** — very fast, but no persistent storage or metadata |
| **PG Vector** | Adds vector search to **PostgreSQL**; popular when a Postgres database already exists |
| **Qdrant** | Production-grade open-source vector database |

- **Common doubt:** **FAISS** stands for *Facebook AI Similarity Search* — it is only the **search index**, so it does not store documents or metadata the way Chroma does.
- **Industry pattern:** many teams already run **PostgreSQL**, so they add **PG Vector** rather than introduce a brand-new database.

### Activity — Pick a database

Write **one reason** you would choose **Chroma** over **Pinecone** for this workshop, and one reason a large company might choose **PG Vector**.

---

## Common Points of Failure in a RAG System

A RAG answer passes through many stages, so a mistake in **any one stage** makes the final answer wrong. Knowing these stages helps you debug quickly (and this is a frequent **interview question**).

- **Chunking** — too few or too many chunks, or a wrong chunk size, loses important context.
- **Embedding model** — a weak model creates poor vectors, so the right chunk is never retrieved.
- **Vector database & metadata** — stale data (documents added or removed but **not updated**) returns wrong results.
- **Number of chunks retrieved (top-k)** — too few = missing context; too many = **higher cost** and **leaking private data** to the LLM.
- **LLM system message** — unclear grounding rules let the model answer from **memory** instead of the context.

### Activity — Spot the failing stage

For the question *"What is the warranty period?"*, the app gives a wrong answer. Write **which stage** you would check **first** and why.

---

## Handling Images in Documents — Multimodal RAG

Real manuals often contain **diagrams and photos**, not just text. RAG can handle these too.

- **Official Definition:** **Multimodal RAG** stores and retrieves both **text** and **images** as embeddings in the same vector database.
- **In Simple Words:** Just like text becomes index cards, **each picture becomes its own card** with its own meaning-vector.
- **Real-Life Example:** A washing-machine manual where the **error-light diagram** matters as much as the written steps.

**How it works:**

- Use a library like **PyMuPDF** to pull the **images** out of a PDF.
- Create **image embeddings** with an **image embedding model** (text uses a text model; images use an image model).
- Store **image chunks** beside **text chunks** in the same vector DB — every record is still just a chunk.
- If a picture contains text, **OCR (Optical Character Recognition)** reads that text out automatically, so it becomes searchable.

### Activity — Find a useful image

Name **one image** in any product manual you own (e.g. an error-code chart) that would help answer a support question, and write the question it would answer.

---

## Turning Your App into a Web Demo with Gradio

A command-line app is great for testing, but a **web interface** makes it easy for anyone to try.

- **Official Definition:** **Gradio** is a Python library that wraps a function in a simple **web interface** with input and output boxes.
- **In Simple Words:** It gives your RAG function a **website with a text box**, so users ask questions without touching code.
- **Real-Life Example:** Like a **company website search bar** — type a question, get an answer.

```python
import gradio as gr  # Web UI library

# Note: build embed_model and collection first (see main() above), then reuse them here.

def rag_qa(query):  # Function Gradio calls for every question
    result = rag_answer(query, collection, embed_model, top_k=TOP_K)  # Run retrieve -> prompt -> generate
    return result["answer"]  # Send only the final answer to the screen

demo = gr.Interface(  # Build the web interface
    fn=rag_qa,  # Function to run on each submit
    inputs="text",  # A single text box for the user's question
    outputs="text",  # A text box to show the answer
    title="ShopEasy Support Assistant",  # Heading shown on the page
)

demo.launch(share=True)  # Start the app and create a shareable link
```

**How the code works:**

- **`rag_qa`** takes the typed question, runs the full RAG pipeline, and returns the answer.
- **`gr.Interface`** connects that function to a text box in and a text box out.
- **`demo.launch(share=True)`** starts a small web server and gives a link you can share for a quick demo.

- **Common doubt / limit:** a free share link runs on a **small server**, so many users at once make it slow; real deployment needs **load balancing**, **auto-scaling**, and **request queuing**, usually handled by a separate infrastructure team.
- **Streamlit** is another option that is generally more **robust and secure** for slightly larger demos.

### Activity — Add a title

Change the `title` in the Gradio interface to a name that fits your own corpus, then describe in one line who would use this web app.

---

## Evaluating a RAG System — LLM as a Judge

Building the app is only half the job; you must also check whether the answers are **trustworthy**. This matters most for enterprises whose technicians act on the answers.

- **Official Definition:** **LLM-as-a-Judge** uses a **second language model** to score the answer produced by the first model.
- **In Simple Words:** One model **answers the exam**, and a **stricter examiner** model **grades** that answer.
- **Real-Life Example:** A performer competes, and a **judge** (who must be more capable) rates the performance.

**The two main metrics:**

- **Groundedness** — is the answer **supported by the retrieved context**? In simple words: did it answer from the notes, not from memory?
- **Relevance** — is the answer **relevant to the question**? In simple words: a *revenue* question should get a *revenue* answer, not a *profit* answer.

**How the judge works:**

- The judge LLM receives the **original question**, the **retrieved context**, and the **generated answer**.
- It returns a rating, usually on a **1 to 5 scale**, guided by its **own system message**.
- Run it on a **small sample** (e.g. 10–15 handpicked questions), not every query, because each judge call **costs money**.
- The judge should ideally be a **more powerful model** than the one that wrote the answer (in the demo the same Llama 70B model is reused for simplicity).

**The RAG Triad:** a framework that adds a third check on top of groundedness and relevance.

- **Context relevance** — are the **retrieved chunks** themselves relevant to the question? This checks the **quality of retrieval**.
- Together, **groundedness + relevance + context relevance** form the **RAG Triad** (a common interview term).
- **Ragas** is a ready-made evaluation library, but teams often **build their own judges** for more **control**.

```python
# Groundedness judge — rates whether the answer comes only from the context
# Note: reuse the same Groq client and GROQ_MODEL set up earlier.

GROUNDEDNESS_JUDGE_SYSTEM = (  # The judge's own system message
    "You are tasked with rating AI-generated answers.\n"
    "Metric: the answer must be derived ONLY from the information in the context.\n"
    "Rating scale: give 1 if the metric is not followed at all, and 5 if it is fully followed.\n"
    "Reply with a single number from 1 to 5."
)

def judge_groundedness(question, context, answer, client):  # Score one answer
    user_message = (  # The judge needs all three inputs
        f"Question: {question}\n\nContext: {context}\n\nAnswer: {answer}"
    )
    response = client.chat.completions.create(  # Call the judge LLM
        model=GROQ_MODEL,  # A capable model acts as the judge
        messages=[
            {"role": "system", "content": GROUNDEDNESS_JUDGE_SYSTEM},  # Judging rules
            {"role": "user", "content": user_message},  # Question + context + answer
        ],
        temperature=0,  # Keep scoring stable and repeatable
    )
    return response.choices[0].message.content  # The 1-to-5 rating
```

**How the code works:**

- **`GROUNDEDNESS_JUDGE_SYSTEM`** tells the judge what to measure and the 1–5 scale to use.
- **`judge_groundedness`** sends the **question, context, and answer** to the judge model and returns a rating.
- A **relevance judge** is built the same way — only the system message changes to check answer-vs-question relevance.

### Activity — Write a judging rule

Write **one line** of a groundedness rule you would give the judge for a **warranty question**, so it gives a low score when the answer adds facts not present in the context.
...
- **Small corpora** + `source_id` metadata make peer review and grounding checks practical, and the same pattern scales to large real manuals.
- **Delimiters** and **versioned grounding rules** tie answers to retrieved evidence; `TOP_K` and backend are config only.
- **Choosing the right vector DB**, knowing the **points of failure**, and handling **images (multimodal RAG)** turn a demo into a real solution.
- **Evaluation with an LLM-as-a-Judge** (groundedness, relevance, and the RAG Triad) proves your app is trustworthy — no LLM application is complete without evaluation.

This capstone build closes **Module 3** — you now have a runnable RAG app, ways to test and evaluate it, plus the agent skills (tools, memory, structured outputs) to extend it further.
...
| **Chroma / Pinecone / FAISS / PG Vector / Qdrant** | Tools | Vector database options (storage, managed, index-only, Postgres-based, production) |
| **PyMuPDF** | Library | Extracts images (and text) from PDFs for multimodal RAG |
| **OCR** | Concept | Optical Character Recognition — reads text inside images |
| **Gradio / Streamlit** | Library | Build a simple web interface for the RAG app |
| **LLM-as-a-Judge** | Concept | A second LLM scores the first model's answer |
| **Groundedness** | Metric | Is the answer supported by the retrieved context? |
| **Relevance** | Metric | Is the answer relevant to the question? |
| **RAG Triad** | Concept | Groundedness + relevance + context relevance |
| **Ragas** | Library | Ready-made RAG evaluation framework |
